...
function Kpi({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-md border border-[var(--border-gray)] bg-white px-4 py-3">
      ...
      <div className="mt-0.5 text-2xl font-semibold tabular-nums text-[var(--content-text)]">{value}</div>
    </div>
  );
}
...
export default function RunLog({
  ...
  const meta = useMemo(() => {
    ...
    const cost = allRuns.reduce((acc, r) => acc + (Number(r.total_cost_usd) || 0), 0);
    return {
  ...
  const columns =
    view === 'url'
      ? [
          ...
          { key: 'latestEntities', label: 'Latest entities', numeric: true },
          { key: 'runs', label: 'Runs', numeric: true },
          { key: 'minEntities', label: 'Entity range', numeric: true },
      ...
      : [
          ...
          { key: 'url', label: 'URL', numeric: false },
          { key: 'analysis_type', label: 'Type', numeric: false },
  ...
  return (
    <div className="mx-auto max-w-6xl rounded-xl bg-[var(--background-gray)] p-6 text-[var(--content-text)]">
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-5">
        ...
        <Kpi label="Total cost" value={meta.cost} />
      </div>
            ...
            <tr>
              {columns.map((c) => (
                <th
                  ...
                  className={`cursor-pointer select-none whitespace-nowrap border-b border-[var(--border-gray)] px-3.5 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-[var(--muted-text)] hover:text-[var(--content-text)] ${
            ...
            {view === 'url'
              ? (shown as unknown as UrlSummary[]).map((r) => (
                  <tr key={r.url} className="border-b border-[var(--light-gray)] last:border-b-0 hover:bg-[var(--light-gray)]">
                    <td className="max-w-[480px] break-all px-3.5 py-1.5 font-mono text-xs">{r.url}</td>
                    <td className="px-3.5 py-1.5 text-right font-mono text-xs tabular-nums">{r.latestEntities}</td>
                    <td className="px-3.5 py-1.5 text-right font-mono text-xs tabular-nums">{r.runs}</td>
                    <td className="px-3.5 py-1.5 text-right font-mono text-xs tabular-nums">
                      {r.minEntities === r.maxEntities ? r.minEntities : `${r.minEntities}–${r.maxEntities}`}
                    </td>
                    <td className="px-3.5 py-1.5 text-right font-mono text-xs tabular-nums">{usd(r.totalCost)}</td>
                    <td className="whitespace-nowrap px-3.5 py-1.5 font-mono text-xs tabular-nums text-[var(--secondary-content)]">
                      {r.lastRun}
                    </td>
                  </tr>
                ))
              : (shown as unknown as AnalysisRun[]).map((r, i) => (
                  ...
                  >
                    ...
                    <td className="max-w-[420px] break-all px-3.5 py-1.5 font-mono text-xs">
                      {r.url ?? '(pasted content)'}
                    </td>
                    <td className="px-3.5 py-1.5 text-xs">{r.analysis_type}</td>
                    <td className="px-3.5 py-1.5 text-xs">{r.key_source === 'byok' ? 'BYOK' : 'app'}</td>
                    <td className="px-3.5 py-1.5"><StatusChip status={r.status} /></td>
                    <td className="px-3.5 py-1.5 text-right font-mono text-xs tabular-nums">{r.entities_found}</td>
                    <td className="px-3.5 py-1.5 text-right font-mono text-xs tabular-nums">
